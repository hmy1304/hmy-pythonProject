...
class NaverProductScraper:
    NAVER_API_PRODUCT = "https://openapi.naver.com/v1/search/shop"
    ...
    def unit_url(self, keyword, start):
        return {
            "url": f"{self.NAVER_API_PRODUCT}?query={keyword}&display=10&start={start}",
            "headers": {
    ...
    async def search(self, keyword, total_page):
        ...
        async with aiohttp.ClientSession() as session:
            all_data = await asyncio.gather(
                *[
                    NaverProductScraper.fetch(session, api["url"], api["headers"])
                    for api in apis
                ]
            )
            result = []
